...
- **Tested.** 72 unit tests, plus a 53-assertion end-to-end test against real Coolify. It installs Coolify, deploys a real app through the API and loads it through the proxy, upgrades, wipes the server, reinstalls, restores, and proves the original login, data, API token and app came back.
...
make test        # 72 unit tests, no Docker or network needed
