# ATTa: Coolify deploy kit for ATT

This repo sets up a [Coolify](https://coolify.io) server that hosts **ATT**.
Coolify is a self-hosted platform, similar to Heroku or Vercel, that builds and runs your apps on your own server.
...
- **Tested.** 57 unit tests, plus a 45-assertion end-to-end test against real Coolify. It installs Coolify, deploys a real app through the API and loads it through the proxy, upgrades, wipes the server, reinstalls, restores, and proves the original login, data, API token and app came back.
...
**Follow [docs/RUNBOOK.md](docs/RUNBOOK.md) from step 6** to add a domain and HTTPS, close the temporary ports and set up backups.
...
| `docs/DEPLOY-ATT.md` | How ATT gets deployed, and the details needed from the ATT codebase. |
...
make test        # 57 unit tests, no Docker or network needed
