"""app_owners.py — who added each library app, so per-app data is only shown to them (and admins).

v115. Written by the pipeline when an upload or git address adds a NEW app to the library
(state/app_owners.json, root 0600). Read by the gateway before it serves an app's stage-6 evidence or
takes customer secrets for it. An app with no record (in the library before v115, or a seed repo) is
admin-only: nobody is guessed as its owner. An admin assigns one on the server:
...
def record(app: str, owner: str, build_id: str | None, replace: bool = False) -> None:
    """First owner wins (the library copy is authoritative, and so is who added it) unless replace=True."""
    aid = app_id(app)
