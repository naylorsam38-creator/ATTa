...
# v112: keep what the browser actually saw for every app, pass or fail. false = no files kept.
CAPTURE_EVIDENCE=os.environ.get('APP_BUILDER_CAPTURE_EVIDENCE','true').lower() in {'1','true','yes'}
# Where results go. One JSON line per app per sweep.
ROOT=Path(os.environ.get('APP_BUILDER_ROOT','/srv/app-builder'))
# v112: per-app browser evidence lives here: <app>/screenshot.png, page.html, browser.json (console + failed requests).
...
def _capture(app,page,console_errors,request_failures,errors):
    """v112 evidence: what stage 6 saw, written before any verdict so a FAIL keeps its picture too.
    Never fails the stage: a capture problem is recorded on the result, not turned into a verdict."""
    if not (CAPTURE_EVIDENCE and app): return None
    d=EVIDENCE_DIR/re.sub(r'[^a-z0-9-]+','-',str(app).lower()).strip('-')
    try:
        d.mkdir(parents=True,exist_ok=True)
        page.screenshot(path=str(d/'screenshot.png'),full_page=True)
        (d/'page.html').write_text(page.content(),errors='replace')
        (d/'browser.json').write_text(json.dumps({'url':page.url,'title':page.title(),'captured_at':time.time(),
            'page_errors':errors[:50],'console_errors':console_errors[:50],'request_failures':request_failures[:50]},indent=2,default=str)+'\n')
        return {'dir':str(d),'screenshot':str(d/'screenshot.png'),'html':str(d/'page.html'),'browser':str(d/'browser.json')}
    except Exception as e:
        return {'dir':str(d),'error':f'{type(e).__name__}: {e}'}

def browser_check(url, app_url=None, expected=None, app=None):
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            _TL.evidence=_capture(app,page,console_errors,request_failures,errors)
...
def check(t):
    ...
    try:
        ...
        exp=adapter_expected(t); _TL.seen=None; _TL.evidence=None
        stages['6 CLEAN']=browser_check(proxy,app_url,exp,app=app)
        if getattr(_TL,'evidence',None): stages['6 CLEAN']['evidence']=_TL.evidence
        r=result(app,stages)
