...
def qualify(only=None, previous=None):
    ...
    import system_watcher as w, app_runner, rule_lifecycle
    ...
    step('QUALIFYING',apps=len(todo),parallel=app_runner.PARALLEL or len(todo))
    # The app runner checks every app at the same time (APP_BUILDER_RUN_PARALLEL only if set to cap it).
    ...
    for r in results:
        ...
        apps.append({'app':r['app'],'verdict':r['verdict'],'qualified':why is None,'ui_dir':t.get('ui_dir'),
                     ...
                     'evidence':r.get('stages',{}).get('6 CLEAN',{}).get('evidence')})
    ...
    # (rewritten again below once regressions and rule suspects are known)
    if not cl['complete']:
    ...
    # Learned-rule lifecycle. A full run of the WHOLE catalogue (only is None) is the only thing that can
    # confirm a candidate rule; a regression names every rule learned or promoted since the last clean run.
    if only is None:
        life=rule_lifecycle.after_full_run(run_started,cl.get('regressions') or [],cl['complete'])
        cl['rules_promoted']=life['promoted']; cl['rule_suspects']=life['suspects']
        if cl.get('regressions') and life['suspects']:
            problems.append('REGRESSION SUSPECTS (learned rules changed since the last clean run): '+', '.join(life['suspects']))
...
def ingest_upload(stage,bid,owner,original):
    """An uploaded zip that isn't an ATTa bundle holds one app or several. v112: every app in it is
    DISCOVERED (app_discovery.py: any depth, loose files ignored, a repo split into client/server stays
    one app) and each goes into the library under its own name, as a git snapshot, catalogued like any
    other app and queued for the same parallel qualification as the rest of the library."""
    step('DISCOVERING',source=original or stage.name)
    found=app_discovery.discover(stage)
    if not found:
        raise RuntimeError(f'"{original or stage.name}" is not an ATTa bundle and has no app code (no package.json, go.mod, pyproject.toml, composer.json, Dockerfile, ...)')
    for a in found:
        if a['rel']=='.': a['name']=app_name(original or 'app')   # loose files at the top: the zip's own name
    builds.update(bid,apps_discovered=[{k:a[k] for k in ('name','rel','how')} for a in found])
    print(f'discovered {len(found)} app(s): '+app_discovery.summary(found),flush=True)
    added=[]; kept=[]
    for a in found:
