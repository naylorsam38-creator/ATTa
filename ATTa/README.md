...
3. **Add an app** (the "Add app" page): upload any app's source zip (e.g. `gitea-main.zip`),
   or paste a git address (`https://github.com/owner/name`). Uploading the ATTa bundle itself (**admin accounts only**; anyone else's is refused and nothing changes)
   updates the system — skins, Front Door page **and** the code, through ADM (docs/ADM.md), with
   a backup and automatic rollback if the new version doesn't come up. Each one starts a build that you own.
...
- **Apps are started by app_runner.py (v109). Every app in an upload is found and queued on its own (v112, docs/history/CHANGES-2026-09-25-V112.md).** See docs/history/CHANGES-2026-09-24-APP-RUNNER.md.
- **Learned rules have a lifecycle (v110).** candidate → confirmed → degraded → retired, promoted only by a clean full run; see docs/history/CHANGES-2026-09-25-RULE-LIFECYCLE.md and `state/checklists/rule_health.md`.
