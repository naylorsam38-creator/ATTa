...
function customersFor(appId) {
  ...
  for (const c of fs.readdirSync(dir)) {
    ...
    if (fs.existsSync(f)) map[c] = { skin: JSON.parse(fs.readFileSync(f, 'utf8')).layout };
  }
  return map;
}

function build(id, { src, recipeDir, intake, distName = id }) {
  ...
  if (intake && intake.sourcePath && /\.zip$/i.test(intake.sourcePath) && fs.existsSync(intake.sourcePath)) {
    const tmp = path.join(ROOT, 'dist', `.${distName}-unzip`);
  ...
  // ATTA_PROXY_FILE: what-if runs with a candidate proxy.js (results go to DEPLOY-<suffix>.md; the shipped bundle is rebuilt normally).
  fs.copyFileSync(process.env.ATTA_PROXY_FILE || path.join(ROOT, 'ui-bridge', 'proxy.js'), path.join(cap, 'ui-bridge', 'proxy.js'));
  ...
  const zip = path.join(ROOT, 'dist', `${distName}.zip`);
  fs.rmSync(zip, { force: true });
  execSync(`zip -qr ${JSON.stringify(zip)} ${JSON.stringify(distName)}`, { cwd: path.join(ROOT, 'dist') });
...
function waitHttp(url, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    const tick = () => {
      const retry = (why) => {
        if (Date.now() > end) reject(new Error(`no healthy answer on ${url} after ${timeoutMs / 1000}s (${why})`));
        else setTimeout(tick, 2000);
      };
      // The proxy answers 502 while the app behind it is still starting; wait for a non-5xx.
      http.get(url, (r) => { r.resume(); r.statusCode < 500 ? resolve(r.statusCode) : retry(`HTTP ${r.statusCode}`); })
        .on('error', (e) => retry(e.code || e.message));
    };
    tick();
  });
}
...
function sandboxBuildOverride(dist, project, env) {
  ...
  for (const [name, svc] of Object.entries(cfg.services || {})) {
    ...
    for (const raw of lines) {
      // Sandbox: GitHub zip downloads are blocked, so every composer install here comes from git source.
      const line = raw.replace(/\bcomposer (create-project|update|require|install)\b/g, 'composer $1 --prefer-source');
      outLines.push(line);
      if (/^\s*FROM\s/i.test(line)) {
        const noShell = /distroless|scratch|surrealdb|chainguard/i.test(line);
        outLines.push('COPY --from=sandboxca ca-bundle.crt /etc/ssl/certs/ca-certificates.crt',
          // This sandbox only allows HTTPS egress: move Debian/Alpine package mirrors off plain http.
          ...(noShell ? [] : ["RUN for f in /etc/apt/sources.list /etc/apt/sources.list.d/*.sources /etc/apk/repositories; do [ -f \"$f\" ] && sed -i 's#http://#https://#g' \"$f\"; done; true"]),
    ...
    // Sandbox: git-source installs leave a .git in every vendor package; drop them in the same RUN
    // (so they never reach an image layer) or the verify images outgrow this sandbox's disk.
    const withCleanup = [];
    let inComposerRun = false;
    for (const line of outLines) {
      if (/^\s*RUN\s/i.test(line)) inComposerRun = false;
      if (/\bcomposer (create-project|update|require|install)\b/.test(line)) inComposerRun = true;
      if (inComposerRun && !/\\\s*$/.test(line)) {
        withCleanup.push(line + " && (find / -xdev -path '*/vendor/*' -type d -name .git -prune -exec rm -rf {} + 2>/dev/null; true)");
        inComposerRun = false;
      } else withCleanup.push(line);
    }
    const verifyDf = path.join(path.dirname(df), 'Dockerfile.verify');
    fs.writeFileSync(verifyDf, withCleanup.join('\n'));
...
async function verify(id, { dist, skins, pagePath = '/', timeout = 600, log }) {
  ...
  try {
    ...
    // Images and build cache are only needed for the check; drop them so the next bundle has disk.
    compose(['down', '-v', '--remove-orphans', '--rmi', 'all'], 300000);
    spawnSync('docker', ['builder', 'prune', '-af'], { timeout: 300000 });
    sandbox.cleanup();
  }
}
