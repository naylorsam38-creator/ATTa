...
async function runChecks({ proxyUrl, appUrl, appId, pagePath = '/', outDir, skins }) {
  ...
  try {
    ...
    // Apps may still be covered by a splash/overlay (e.g. a service-worker reload right after start):
    // retry for up to 10s before concluding nothing is hittable.
    const pick = async (kind) => {
      let t = { total: 0, kindTotal: 0 };
      for (let i = 0; i < 20; i++) {
        t = await page.evaluate(pickTarget, [PICKABLE, kind]).catch(() => t);
        if (t.x !== undefined || (i >= 2 && !t.kindTotal)) break;
    ...
    const counts = slotOk ? await pick('any') : { total: 0 };
    ...
    for (const [id, kind, name] of [['C6', 'link', 'Drag-move a link'], ['C7', 'button', 'Drag-move a button']]) {
      const t = slotOk ? await pick(kind) : { kindTotal: 0 };
      if (t.x === undefined) {
        record(id, name, 'skip', t.kindTotal ? `${t.kindTotal} on page, none hittable in the viewport` : `no ${kind}s on this page`);
        continue;
      }
      // Pages animate in (entrance/reveal effects); wait until the control has stopped moving
      // so the "before" position is its real resting place.
      for (let i = 0, last = null; i < 16; i++) {
        const r = await page.evaluate(() => { const e = document.querySelector('[data-atta-target]'); const b = e.getBoundingClientRect(); return { left: b.left, top: b.top, w: b.width, h: b.height }; });
        if (last && Math.abs(r.left - last.left) < 0.5 && Math.abs(r.top - last.top) < 0.5) {
          Object.assign(t, { left: r.left, top: r.top, x: r.left + r.w / 2, y: r.top + r.h / 2 });
          break;
        }
        last = r;
      ...
      await page.mouse.up();
      // Move the pointer off the control (apps lift/scale things on :hover), then let the app's
      // own CSS transitions (e.g. `transition: all .15s`) finish before measuring.
    ...
    if (slotOk) await setPanel(false);
    const renameTarget = slotOk ? await pick('any') : { total: 0 };
    let before = null;
    ...
    // Deploy mode has no direct-to-app baseline, so an error there can't be pinned on the Port.
    record('C12', 'No new uncaught errors with the Port', added.length ? (appUrl ? 'fail' : 'warn') : 'pass',
      added.length ? added[0].split('\n')[0].slice(0, 200) + (appUrl ? '' : ' (no direct-to-app baseline in deploy mode — may be the app\'s own)') : `none (${pageErrors.length} uncaught error(s) seen, all also raised by the app without the Port)`);
    ...
    // …or at an internal service name (containers: "http://sendportal/…" — no dot, unreachable for a browser).
    const upstreamRefs = await page.evaluate((up) => [...document.querySelectorAll('a[href],form[action],link[href],script[src],img[src],base[href]')]
      .map((e) => e.getAttribute('href') || e.getAttribute('action') || e.getAttribute('src') || '')
      .filter((u) => (up && u.startsWith(up)) || (/^https?:\/\/([^/:.]+)(:\d+)?(\/|$)/i.test(u) && !/^https?:\/\/localhost/i.test(u))).length, appUrl).catch(() => 0);
    const bits = [];
    ...
    if (upstreamRefs) bits.push(`${upstreamRefs} link/form/asset URL(s) in the page point at <upstream>${appUrl ? '' : ' (internal service hostname)'}`);
    ...
    const skinResults = [];
    for (const [customer, entry] of Object.entries(skins || {})) {
      const skin = typeof entry === 'string' ? entry : entry.skin;
      // Each customer in their own fresh browser (as in reality): no service worker or cache from the other checks.
      const cctx = await browser.newContext({ viewport: { width: 1280, height: 800 }, ignoreHTTPSErrors: true,
        extraHTTPHeaders: { 'x-authenticated-customer-id': customer } });
      const cpage = await cctx.newPage();
      await cpage.goto(proxyUrl + pagePath, { waitUntil: 'load' }).catch(() => {});
      await cpage.waitForLoadState('networkidle', { timeout: 6000 }).catch(() => {});
      const got = await cpage.evaluate(([appId, skin]) => {
        ...
        return { link: !!link, rules, bg: getComputedStyle(document.body).backgroundColor };
      }, [appId, skin]).catch(() => ({}));
      await cctx.close();
      skinResults.push({ customer, skin, label: entry.label || skin, ok: got.link && got.rules > 0, ...got });
    }
    ...
    if (!skinResults.length) record('C15', 'Customer skins through the proxy', 'skip', 'no skins configured');
      ...
      skinResults.map((r) => `${r.customer}→${r.skin} "${r.label}": ${r.ok ? `applied (${r.rules} rules)` : `NOT applied (link=${r.link}, rules=${r.rules})`}`).join('; ') +
